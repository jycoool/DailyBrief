    ...
    for tag, url in EP_INCOME.items():
        ...
        for row in data:
            ...
            if not code:
                continue
            rev = _f(row.get("營業收入"))
            gross = _f(row.get("營業毛利(毛損)", row.get("營業毛利(毛損)淨額")))
            net = _f(row.get("本期淨利(淨損)",
                             row.get("本期稅後淨利（淨損）",
                                     row.get("稅後淨利"))))
            ...
            period = str(row.get("年度", "")) + " " + str(row.get("季別", ""))
            if gm is not None or npm is not None:
                out[code] = {"gm": gm, "npm": npm, "period": period.strip()}
    return out
...
def build_html(macro, stocks, val, rev, inst, exr, inc, refresh=0):
    now = datetime.now()
...
<tr>
  ...
  <th class="grp" colspan="2">獲利能力（季報）</th>
...
def collect(with_fund=True):
    ...
    if with_fund:
        ...
        print("  抓取除權息預告…")
        exr = fetch_exright()
        print("  抓取綜合損益表（毛利率/淨利率）…")
        inc = fetch_income()
    return macro, stocks, val, rev, inst, exr, inc


def write_html(path, refresh=0, with_fund=True):
    macro, stocks, val, rev, inst, exr, inc = collect(with_fund)
    html = build_html(macro, stocks, val, rev, inst, exr, inc, refresh)
