...
def build_html(macro, stocks, funda, fred_data, refresh=0, pm_data=None, polymarket_data=None):
    ...
    # ── 報價日期：取「美國股市」真實交易日（非系統時間）──
    # 外匯（JPY=X 等）為 24 小時交易，最新 bar 常是隔日，會誤導報價日期；
    # 此處優先以個股、其次以美股指數（^GSPC）的 asof 為準。
    asof = None
    if stocks:
        cand = [d.get("asof") for d in stocks.values() if isinstance(d, dict) and d.get("asof") is not None]
        if cand:
            asof = max(cand)
    if asof is None and macro:
        g = macro.get("^GSPC") or {}
        asof = g.get("asof")
    if asof is None:
        asof = now
...
<header>
  ...
  <div class="stamp">
    <div class="eyebrow">報價日期</div>
    <div class="t num">{asof:%Y-%m-%d}</div>
    <div class="eyebrow" style="margin-top:3px">產生於 {now:%H:%M:%S} · 約 15 分鐘延遲{refresh_note}</div>
  </div>
</header>
